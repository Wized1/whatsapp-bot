...
async function serialize(msg, conn) {
 ...
 if (msg.message) {
  ...
  conn.getFile = async (PATH, returnAsFilename) => {
   let res, filename;
   let data = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await axios.get(PATH, { responseType: 'arraybuffer' })).data : existsSync(PATH) ? ((filename = PATH), readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0);
