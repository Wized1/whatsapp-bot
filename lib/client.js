...
const { getPausedChats, loadMessage, saveMessage, saveChat, getName } = require('../db');
const { commands } = require('./plugins');
const { Message, Greetings } = require('../client');
...
const connect = async () => {
 ...
 const handleMessages = async ({ messages }) => {
  ...
  const isResume = new RegExp(`${config.PREFIX}( ?resume)`, 'is').test(msg.body);
 ...
 const greetingsHandler = new Greetings(client);
